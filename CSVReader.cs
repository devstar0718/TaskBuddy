...
namespace TaskBuddy
{
    class CSVReader : IDisposable
    {
            ...
            public CSVReader(FileInfo csvFileInfo)
            {
                ...
                {
                    //System.Windows.Forms.MessageBox.Show("error opening parts list CSV " + ex.ToString());
                    Console.WriteLine("error opening parts list CSV " + ex.ToString());
                }
            }
            ...
            private object ReadNextObject()
            {
                ...
                if (quoted)
                {
                    if (i > len || !currentLine.Substring(i, 1).StartsWith("\""))
                        throw new System.FormatException("Invalid CSV format: " + currentLine.Substring(0, i));
                    i++;
                }
            ...
            public DataTable CreateDataTable(bool headerRow)
            {
                ...
                if (rows.Count < 1) return new DataTable();
                ...
                for (int i = 0; i < columnTypes.Count; i++)
                {
                    ...
                    if (i < columnNames.Count)
                    {
                        ...
                        {
                            table.Columns[i].ColumnName = @"Column #" + i.ToString();
                        }
                    }
                }
